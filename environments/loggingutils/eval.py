...
def evaluate(nsteps, eval_envs, agent, NUM_STEPS, DO_VIDEO, VIDEO_SAVE_DIR=None):
    if VIDEO_SAVE_DIR is None:
        VIDEO_SAVE_DIR = wandb.run.dir
    ...
    for _ in tqdm(range(0, NUM_STEPS)): # todo this should match train_env's max ep len to some extent
        ...
        next_obs, _, term, trunc, info = eval_envs.step(action)
    ...
    if DO_VIDEO:
        ...
        out = cv2.VideoWriter(f'{VIDEO_SAVE_DIR}/{nsteps}_{eval_envs.ONEIROS_METADATA.prefix}.mp4', cv2.VideoWriter_fourcc(*'mp4v'), FPS, (SHAPE[1], SHAPE[0]))
